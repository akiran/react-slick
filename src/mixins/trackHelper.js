...
export var getTrackLeft = function (spec) {
  ...
  if (spec.variableWidth === true) {
      ...
      if(spec.slideCount <= spec.slidesToShow || spec.infinite === false) {
          targetSlide = findDOMNode(spec.trackRef).childNodes[spec.slideIndex];
      } else {
          targetSlideIndex = (spec.slideIndex + spec.slidesToShow);
          targetSlide = findDOMNode(spec.trackRef).childNodes[targetSlideIndex];
      }
      targetLeft = targetSlide ? targetSlide.offsetLeft * -1 : 0;
      if (spec.centerMode === true) {
          if(spec.infinite === false) {
              targetSlide = findDOMNode(spec.trackRef).children[spec.slideIndex];
          } else {
              targetSlide = findDOMNode(spec.trackRef).children[(spec.slideIndex + spec.slidesToShow + 1)];
          }
